...
class CronPerDay extends Command
{
    ...
    public function handle()
    {
        try {
            ...
            // 6. INSERT DATA PENERIMAAN 118 TO DATA PENERIMAAN LAIN (Memindahkan Rincian Transaksi Penerimaan Ambulan 118 (H+1) berdasarkan kwitansi (bukti bayar pihak pembayar))
            $run('6. Insert Data Penerimaan 118 To Data Penerimaan Lain', function () use ($yesterday) {
                // Get data penerimaan 118 yesterday
                $penerimaan118Siesta = (new DataPenerimaanLain)->setTable('simpra_penerimaan118_ft')->whereDate('tgl_bayar', $yesterday)->get();
                $inserted = 0;
                foreach ($penerimaan118Siesta as $p118) {
                    // Check if data already exist in table data_penerimaan_lain (based on no_bayar)
                    $exist = DataPenerimaanLain::where('no_bayar', $p118->no_bayar)->exists();
                    // Insert if data not exist yet
                    if (!$exist) {
                        DataPenerimaanLain::create($p118->toArray());
                        $inserted++;
                    }
                }
                // log inserted data
                \Log::info("Cronjob no 6 berhasil insert {$inserted} data ke table data_penerimaan_lain.");
            });

            // 7. INSERT DATA POTENSI PELAYANAN (Memindahkan Rincian Transaksi Potensi Pelayanan (H+2) berdasarkan tanggal pelayanan (jika rawat jalan tanggal pendaftaran sedangkan rawat inap tanggal KRS))
            $run('7. Insert Data Potensi Pelayanan', function () use ($two_days_ago) {
                // Get data potensi pelayanan yesterday (SEMENTARA PAKE ACUAN TGL_PELAYANAN)
                $potensiPelayananSiesta = (new DataPotensiPelayanan)->setTable('simpra_potensipelayanan_ft')->whereDate('tgl_pelayanan', $two_days_ago)->get();
                $inserted = 0;
            ...
            // 8. INSERT DATA DOKUMEN NON-LAYANAN (Memindahkan Rincian Transaksi Potensi Lainnya (H+1) berdasarkan tanggal_dokumen (tanggal surat pengesahan invoice))
            $run('8. Insert Data Dokumen Non-Layanan', function () use ($yesterday) {
                ...
                // log inserted data
                \Log::info("Cronjob no 8 berhasil insert {$inserted} data ke table dokumen_nonlayanan.");
            });

            // 9. INSERT DATA REKAP HARIAN CARA BAYAR (Memindahkan Data Rekapitulasi Harian Tagihan Pelayanan per Cara Bayar (H+28) berdasarkan tanggal pelayanan)
            $run('9. Insert Data Rekap Harian Cara Bayar', function () use ($twenty_eight_days_ago) {
                ...
                // log inserted data
                \Log::info("Cronjob no 9 berhasil insert {$inserted} data ke table data_rekap_harian_carabayar.");
            });

            // 10. SYNC STATUS FASE 1 ON DATA PENDAPATAN PELAYANAN (Sinkronisasi data tagihan (H+4) berdasarkan tanggal pelayanan)
            $run('10. Sync Status Fase 1 On Data Pendapatan Pelayanan', function () use ($four_days_ago) {
                ...
                // log updated data
                \Log::info("Cronjob no 10 berhasil update {$updated} data pada table data_pendapatan_pelayanan.");
            });

            // 11. SYNC STATUS FASE 2 ON DATA PENDAPATAN PELAYAAN (PAKAI DARI MODEL DATA PENDAPATAN PELAYANAN)
            ...
            $run('11. Sync Status Fase 2 On Data Pendapatan Pelayanan', function () {
                $syncFase2All = DataPendapatanPelayanan::syncFase2All();
                // log data
                \Log::info('Cronjob no 11 berhasil dijalankan', $syncFase2All);
            });

            // 12. SYNC POTENSI ID, POTENSI NO AND POTENSI NOMINAL ON DATA PENDAPATAN PELAYANAN (Sinkronisasi klarifikasi piutang penjaminan pada data tagihan (H+30) berdasarkan tanggal pelayanan)
            $run('12. Sync Potensi Id, Potensi No and Potensi Nominal on Data Pendapatan Pelayanan', function () use ($thirty_days_ago) {
                ...
                // log updated data
                \Log::info("Cronjob no 12 berhasil update {$updated} data pada table data_pendapatan_pelayanan.");
            });
